                    ...
                    opengl_program_common *Result)
{
    ...
    Result->VertTextureIndex = glGetAttribLocation(ProgramID, "VertTextureIndex");
...
UseProgramBegin(opengl_program_common *Prog)
{
    ...
    if(IsValidArray(TextureIndex))
    {
        ...
        glVertexAttribIPointer(TextureIndex, 1, GL_UNSIGNED_SHORT, sizeof(textured_vertex), (void *)OffsetOf(textured_vertex, TextureIndex));
    }
...
internal void
CompileZBiasProgram(open_gl *OpenGL, zbias_program *Result, b32x DepthPeel, b32x LightingDisabled)
{
    ...
    uniform sampler3D LightIndexSampler;
        ...
        v3 SumLight()
    {
              ...
              v3 VoxIndexP = InvVoxelCellDim*VoxRelP;
              
int VoxX = int(floor(VoxIndexP.x));
    int VoxY = int(floor(VoxIndexP.y));
    int VoxZ = int(floor(VoxIndexP.z));
    
    float tX = fract(VoxIndexP.x);
    float tY = fract(VoxIndexP.y);
    float tZ = fract(VoxIndexP.z);
    
int LightI000 = texelFetch(LightIndexSampler, ivec3(VoxX + 0, VoxY + 0, VoxZ + 0), 0);
    int LightI001 = texelFetch(LightIndexSampler, ivec3(VoxX + 1, VoxY + 0, VoxZ + 0), 0);
    int LightI010 = texelFetch(LightIndexSampler, ivec3(VoxX + 0, VoxY + 1, VoxZ + 0), 0);
    int LightI011 = texelFetch(LightIndexSampler, ivec3(VoxX + 1, VoxY + 1, VoxZ + 0), 0);
        int LightI100 = texelFetch(LightIndexSampler, ivec3(VoxX + 0, VoxY + 0, VoxZ + 1), 0);
    int LightI101 = texelFetch(LightIndexSampler, ivec3(VoxX + 1, VoxY + 0, VoxZ + 1), 0);
    int LightI110 = texelFetch(LightIndexSampler, ivec3(VoxX + 0, VoxY + 1, VoxZ + 1), 0);
    int LightI111 = texelFetch(LightIndexSampler, ivec3(VoxX + 1, VoxY + 1, VoxZ + 1), 0);
    
    v4 LightData000 = texelFetch(LightSampler, LightI000, SideI, 0);
    v4 LightData001 = texelFetch(LightSampler, LightI001, SideI, 0);
    v4 LightData010 = texelFetch(LightSampler, LightI010, SideI, 0);
    v4 LightData011 = texelFetch(LightSampler, LightI011, SideI, 0);
    v4 LightData100 = texelFetch(LightSampler, LightI100, SideI, 0);
    v4 LightData101 = texelFetch(LightSampler, LightI101, SideI, 0);
    v4 LightData110 = texelFetch(LightSampler, LightI110, SideI, 0);
    v4 LightData111 = texelFetch(LightSampler, LightI111, SideI, 0);
    
    v4 LightData00 = mix(LightData000, LightData001, tX);
        v4 LightData01 = mix(LightData010, LightData011, tX);
            v4 LightData10 = mix(LightData100, LightData101, tX);
                v4 LightData11 = mix(LightData110, LightData111, tX);
                
                v4 LightData0 = mix(LightData00, LightData01, tY);
                v4 LightData1 = mix(LightData10, LightData11, tY);
                
                                v4 LightData = mix(LightData0, LightData1, tZ);
                                
                                        return(LightData.rgb);
    }
    #endif
...
CompilePeelComposite(open_gl *OpenGL, opengl_program_common *Result)
{
    ...
    OpenGLLinkSamplers(Result, "Peel0Sampler", "Peel1Sampler", "Peel2Sampler", "Peel3Sampler");
}
...
CompileResolveMultisample(open_gl *OpenGL, resolve_multisample_program *Result)
{
    ...
    GLuint Prog = OpenGLCreateProgram(Defines, GlobalShaderHeaderCode, VertexCode, FragmentCode, &Result->Common);
    OpenGLLinkSamplers(&Result->Common, "DepthSampler", "ColorSampler", "MaskSampler");
}
...
OpenGLInit(open_gl *OpenGL, opengl_info Info, b32 FramebufferSupportsSRGB)
{
    ...
    for(mip_iterator MIP = IterateMIPs(TEXTURE_ARRAY_DIM, TEXTURE_ARRAY_DIM, 0);
        ...
        Advance(&MIP))
    {
        glTexImage3D(GL_TEXTURE_2D_ARRAY, MIP.Level, OpenGL->DefaultSpriteTextureFormat,
...
OpenGLChangeToSettings(open_gl *OpenGL, game_render_settings *Settings)
{
    ...
    CompileZBiasProgram(OpenGL, &OpenGL->ZBiasNoDepthPeel, false, OpenGL->CurrentSettings.LightingDisabled);
    CompileZBiasProgram(OpenGL, &OpenGL->ZBiasDepthPeel, true, OpenGL->CurrentSettings.LightingDisabled);
    ...
    while((TexWidth > 1) && (TexHeight > 1))
    {
        ...
        Buf->FrontEmitTex = FramebufferTexImage(OpenGL,
                                                GL_TEXTURE_2D, TexWidth, TexHeight, FilterType, GL_RGB32F);
        Buf->BackEmitTex = FramebufferTexImage(OpenGL,
                                               GL_TEXTURE_2D, TexWidth, TexHeight, FilterType, GL_RGB32F);
        Buf->SurfaceColorTex = FramebufferTexImage(OpenGL,
                                                   GL_TEXTURE_2D, TexWidth, TexHeight, FilterType, GL_RGB32F);
        Buf->NPTex = FramebufferTexImage(OpenGL,
                                         GL_TEXTURE_2D, TexWidth, TexHeight, FilterType, GL_RGB32F);
        ...
        else if(Op->State == TextureOp_ReadyToTransfer)
        {
...
#endif
                glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, OpenGL->DefaultSpriteTextureFormat,
            ...
            else
            {
                ...
                for(mip_iterator MIP = IterateMIPs(Texture.Width,
                    ...
                    Advance(&MIP))
                {
                    glTexSubImage3D(GL_TEXTURE_2D_ARRAY, MIP.Level, 0, 0,
...
OpenGLBeginFrame(open_gl *OpenGL, v2u OSWindowDim, v2u RenderDim, rectangle2i DrawRegion)
{
    ...
    Commands->MaxLightProbeCount = OPENGL_MAX_LIGHT_PROBE_COUNT;
    Commands->LightProbeCount = 0;
    Commands->LightProbeIrradiance = OpenGL->LightProbeIrradiance;
    Commands->LightProbePosition = OpenGL->LightProbePosition;
    
    Commands->LightIndexVoxelDim = OPENGL_LIGHT_INDEX_VOXEL_DIM;
    Commands->LightIndexVoxel = OpenGL->LightIndexVoxel;
    
    Commands->MaxLightOccluderCount = OPENGL_MAX_LIGHT_OCCLUDER_COUNT;
        ...
        )
    {
        ...
        switch(Header->Type)
        {
            ...
            case RenderGroupEntryType_render_entry_begin_peels:
            {
                ...
                OpenGLBindFramebuffer(&OpenGL->DepthPeelBuffer, RenderWidth, RenderHeight);
            ...
            case RenderGroupEntryType_render_entry_textured_quads:
            {
                ...
                else
                {
                    ...
                    glBindTexture(GL_TEXTURE_2D_ARRAY, OpenGL->TextureArray);
                    glDrawElementsBaseVertex(GL_TRIANGLES, 6*Entry->QuadCount, GL_UNSIGNED_SHORT,
